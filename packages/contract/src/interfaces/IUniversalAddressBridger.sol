...
/// @notice Simplified bridger interface built for Universal Address stablecoin transfers.
interface IUniversalAddressBridger {
    /// @notice Fetches a quote: what do I have to send in so that $x shows up on the destination?
    ...
    /// @return inAmount       The exact quantity of bridgeTokenIn that must be provided
    function getBridgeTokenIn(
        uint256 toChainId,
        TokenAmount calldata bridgeTokenOut
    ) external view returns (address bridgeTokenIn, uint256 inAmount);

    /// @notice Execute the bridge. Reverts if the adapter can't deliver the specified destination amount.
