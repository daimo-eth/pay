...
contract DaimoPayRelayer is AccessControl {
    ...
    ///      repayment relayer. For ERC20s this function approves the manager
    ///      for the exact amount, calls `fastFinish`, then clears approval.
    ///      For native token it forwards exact `msg.value`.
    function accountVaultFastFinish(
        AccountVaultManager manager,
        AccountVaultParams calldata params,
        TokenAmount calldata tokenIn
    ) public payable onlyRole(DEFAULT_ADMIN_ROLE) {
        if (address(tokenIn.token) == address(0)) {
            require(msg.value == tokenIn.amount, "DPR: wrong value");
            manager.fastFinish{value: tokenIn.amount}(params, tokenIn.token, tokenIn.amount);
            return;
        }

        require(msg.value == 0, "DPR: wrong value");
        tokenIn.token.forceApprove(address(manager), tokenIn.amount);
        manager.fastFinish(params, tokenIn.token, tokenIn.amount);
        tokenIn.token.forceApprove(address(manager), 0);
    }
