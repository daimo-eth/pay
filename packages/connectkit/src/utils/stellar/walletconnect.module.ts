...
import { defineStellarChain, STELLAR_NETWORKS } from "./types";
...
export class WalletConnectModule implements ModuleInterface {
  ...
  private qrModal!: ReturnType<typeof createAppKit>;
  ...
  async signTransaction(
    ...
    return runChecks()
      .then(async () => {
        const targetSession: IParsedWalletConnectSession =
          await this.getTargetSession({ publicKey: opts?.address });
